...
**Install & configure 3 katana2x masternodes:**

```bash
./install.sh -p katana2x -c 3
```

**Update daemon of previously installed KATANA2X masternodes:**

```bash
./install.sh -p katana2x -u
```

**Wipe all katana2x masternode data:**

```bash
./install.sh -p katana2x -w
```

**Install 2 katana2x masternodes and configure sentinel monitoring:**
...
**All donations/reflinks originally for the first developer, we are not willing to stole other's creation but using it good way**
**Thanks to main developer**
